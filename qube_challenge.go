...
func checkActorNames() bool {
	//create a channel which will hold the response
	res := make(chan response)
	throttle := time.Tick(time.Second / 800)

	//run the getData method in async way
	go getdata(os.Args[1], res, throttle)

	//store the res in channel
	r := <-res
	if r.err != nil {
		return false
	}
...
func main() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: degrees <actor-name1> <actor-name2>")
	}
